...
class FirstFtcApi:
    ...
    async def _get(self, path: str, params: dict | None = None) -> dict:
        ...
        if r.status_code in (301, 302, 307, 308):
            raise RuntimeError(f"Unexpected redirect, got {r.status_code} to {r.headers.get('Location')}")

        r.raise_for_status()
        return r.json()

    async def get_event_teams(self, season: int, event_code: str) -> dict:
        return await self._get(f"v2.0/{season}/teams", params={"eventCode": event_code})

    async def get_schedule(self, season: int, event_code: str, tournament_level: str = "qual") -> dict:
        return await self._get(f"v2.0/{season}/schedule/{event_code}/{tournament_level}/hybrid")

    async def get_results(self, season: int, event_code: str) -> dict:
        return await self._get(f"v2.0/{season}/matches/{event_code}")
